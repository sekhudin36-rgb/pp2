...
export default function Sidebar({ onLogout, onOpenAi, isMobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "w-64 flex flex-col gap-4 bg-slate-900/80 backdrop-blur-2xl border border-white/10 rounded-3xl p-5 shrink-0 h-full relative overflow-hidden transition-transform duration-300 z-40",
        // Mobile responsiveness
        "fixed inset-y-4 left-4 lg:static lg:translate-x-0",
        isMobileOpen ? "translate-x-0 shadow-2xl" : "-translate-x-[120%] lg:translate-x-0"
      )}>
