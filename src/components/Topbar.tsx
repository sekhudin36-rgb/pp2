...
export default function Topbar({
  ...
  return (
    <header className="h-16 sm:h-20 flex items-center justify-between px-3 sm:px-8 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl sm:rounded-3xl shrink-0 z-20 w-full relative">
      {/* Left: Title & School identity */}
      <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-2">
            <h2 className="text-base sm:text-xl font-bold text-slate-50 tracking-tight truncate">{libraryName}</h2>
            {/* Dapodik-style Online/Offline Badge */}
            <div 
              className={`hidden md:flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-semibold tracking-wider uppercase border shrink-0 ${
                isOnline 
          ...
          <p className="text-[11px] sm:text-xs text-slate-400 hidden sm:block capitalize truncate">
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
        ...
        {onOpenSearch && (
          <button
            ...
            className="flex items-center gap-1.5 p-2 sm:px-4 sm:py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl sm:rounded-2xl text-xs text-slate-300 transition-all shadow-sm group"
        ...
        {onOpenScan && (
          <button
            onClick={onOpenScan}
            className="p-2 sm:px-3 sm:py-2 rounded-xl sm:rounded-2xl bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/30 text-blue-300 transition-all flex items-center gap-1.5 text-xs font-medium"
        ...
        {onOpenAi && (
          <button
            onClick={onOpenAi}
            className="p-2 sm:px-3 sm:py-2 rounded-xl sm:rounded-2xl bg-indigo-600/20 hover:bg-indigo-600/30 border border-indigo-500/30 text-indigo-300 transition-all flex items-center gap-1.5 text-xs font-medium"
        ...
        <Link
          to="/settings"
          className="p-2 sm:px-3 sm:py-2 rounded-xl sm:rounded-2xl bg-emerald-600/10 hover:bg-emerald-600/20 border border-emerald-500/20 text-emerald-400 transition-all flex items-center gap-1.5 text-xs font-medium"
          title={getStoredToken() ? "Google Drive Terhubung (Klik untuk Cadangkan/Pulihkan)" : "Google Drive (Klik untuk Hubungkan)"}
        >
          <Cloud className="w-4 h-4 text-emerald-400" />
          <span className="hidden xl:inline">{getStoredToken() ? 'Drive Aktif' : 'Hubungkan Drive'}</span>
        </Link>
        ...
        <div className="relative">
          <button
            ...
            className="relative p-2 sm:p-2.5 text-slate-400 hover:text-white rounded-xl sm:rounded-2xl bg-white/5 hover:bg-white/10 transition-colors"
          ...
          {showNotifications && (
            <div 
              className="absolute right-0 mt-3 w-[calc(100vw-1.5rem)] max-w-xs sm:w-80 bg-slate-900/95 border border-white/10 rounded-2xl shadow-2xl p-4 z-50 text-xs backdrop-blur-xl animate-in fade-in zoom-in-95"
